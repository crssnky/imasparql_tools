...
namespace yamlRead {
    class Program {
        static void Main(string[] args) {
            var file = new StreamReader(@"D:\GitJON\utada_data\data\caravan.yml", Encoding.UTF8);
            var deserializer = new DeserializerBuilder().Build();
            var root = deserializer.Deserialize<ComuRoot>(file);

            const string NODE_TITLE = "MillionLiveCaravan";
            var comu = new StreamWriter(@"Communication_MillionLiveCaravan.rdf", false, Encoding.UTF8);
            ...
            using (WebClient webclient = new WebClient()) {
                for (int j = 0; j < root.comu.Count; j++) {
                    var cd = root.comu[j];
                    texts.Clear();
                    comu.WriteLine("<rdf:Description rdf:about=\"detail/" + URIencoding(cd.title) + "\">");
                    comu.WriteLine("<schema:title xml:lang=\"ja\">" + cd.title + "</schema:title>");
                    Console.WriteLine(cd.title);
                    var participant = new List<string>();
                    for (int i = 0; i < cd.texts.Count; i++) {
                        var element = cd.texts[i];
    ...
    public class MillionCDRoot {
        public List<MillionCD> CDs { get; set; }

        public MillionCDRoot() {
            CDs = new List<MillionCD>();
        }
    }
    public class MillionCD {
        ...
        public MillionCD(List<string> list) {
            singers = list;
        }
    }
